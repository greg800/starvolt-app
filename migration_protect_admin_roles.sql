...
-- (permissions, nom, défaut). Un admin standard ne peut toucher aucun de leurs paramètres.

create or replace function public.admin_save_role_permissions(p_key text, p_permissions jsonb)
returns void language plpgsql security definer set search_path to 'public' as $fn$
begin
  if not public.is_admin() then raise exception 'forbidden'; end if;
  if p_key in ('admin','superadmin') and not public.is_superadmin() then raise exception 'forbidden_superadmin'; end if;
